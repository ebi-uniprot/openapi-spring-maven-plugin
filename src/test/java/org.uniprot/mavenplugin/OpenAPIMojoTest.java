...
public class OpenAPIMojoTest {
    ...
    public void testPluginExecuteWithSimplePom() throws Exception {
        ...
        Assert.assertEquals(DEFAULT_TITLE, info.getTitle());
        Assert.assertEquals(DEFAULT_VERSION, info.getVersion());
        List<Server> servers = openAPI.getServers();
        Assert.assertEquals(1, servers.size());
        Assert.assertEquals("localhost", servers.get(0).getUrl());
        Assert.assertEquals(DEFAULT_SERVER_DESCRIPTION, servers.get(0).getDescription());
